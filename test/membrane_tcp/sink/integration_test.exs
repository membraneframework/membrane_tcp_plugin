defmodule Membrane.TCP.SinkIntegrationTest do
  use ExUnit.Case, async: false

  import SocketSetup

  alias Membrane.Buffer
  alias Membrane.TCP.{Endpoint, Sink, Socket}
  ...
  for module <- [Endpoint, Sink] do
    ...
    test "Sends tcp packet through #{inspect(module)}", %{state: state} do
      payload = "A lot of laughs"

      unquote(module).handle_buffer(:input, %Buffer{payload: payload}, nil, state)

      assert_receive {:tcp, _, @local_address, @local_port_no, ^payload}
    end
  end
end
